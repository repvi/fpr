...
//
// USAGE:
//   sprout_init("Extender1");
//   sprout_start();
//   sprout_set_extender_mode();  // Enable routing/forwarding
//

typedef struct {
    sprout_package_type_t package_type;
    sprout_package_id_t package_id;
    uint8_t max_hops;
} sprout_send_options_full_control_t;


static const char *TAG = "sprout_extender";

static bool _should_forward_packet(sprout_package_t *package, const uint8_t *src_mac)
{
    // Don't forward if we're the origin
    if (memcmp(package->origin_mac, sprout_net.mac, 6) == 0) {
        return false;
    }
    ...
    bool is_for_me = (memcmp(package->dest_mac, sprout_net.mac, 6) == 0);
    
    // Forward if broadcast or not for me
    return (is_broadcast_dest || !is_for_me);
}

static esp_err_t sprout_send_data_full_control(uint8_t *peer_address, void *data, int size, const sprout_send_options_full_control_t *options)
{
    ...
    ESP_RETURN_ON_FALSE(size <= sizeof(((sprout_package_t*)0)->protocol), ESP_ERR_INVALID_SIZE, TAG, "Data too large for packet");
    ...
    package.hop_count = 0;
    package.max_hops = options->max_hops > 0 ? options->max_hops : SPROUT_DEFAULT_MAX_HOPS;
...
{
    #if (SPROUT_DEBUG_LOG_EXTENDER_DATA_RECEIVE == 1)
    ...
    SPROUT_STORE_HASH_TYPE *peer = _get_peer_from_map(esp_now_info->src_addr);
    if (peer) {
    ...
    if (sprout_net.routing_enabled && _should_forward_packet(package, esp_now_info->src_addr)) {
        ...
        if (is_broadcast) {
            ...
            SPROUT_STORE_HASH_TYPE *dest_peer = _get_peer_from_map(package->dest_mac);
        ...
        if (next_hop) {
            sprout_send_options_full_control_t options = {
                .package_type = package->package_type,
                .package_id = package->id,
                .max_hops = package->max_hops
            };
            esp_err_t err = sprout_send_data_full_control(next_hop, (void *)&package->protocol, sizeof(package->protocol), &options);
            if (err == ESP_OK) {
                sprout_net.stats.packets_forwarded++;
