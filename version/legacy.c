...
static const char *TAG = "sprout_legacy";

// Expected minimum packet size for v0 packets
// This should match the size of the packet structure from that era
#define SPROUT_V0_MIN_PACKET_SIZE 180  // Approximate, adjust based on actual v0 structure

bool sprout_legacy_is_v0_packet(const uint8_t *data, int len)
{
...
bool sprout_legacy_handle_protocol_v0(const esp_now_recv_info_t *esp_now_info, const uint8_t *data, int len)
{
    ...
    if (len < SPROUT_V0_MIN_PACKET_SIZE) {
        ESP_LOGW(TAG, "Legacy packet too small: %d < %d", len, SPROUT_V0_MIN_PACKET_SIZE);
        return false;
    }
    ...
    // } sprout_package_v0_t;
    //
    // sprout_package_v0_t *old_pkg = (sprout_package_v0_t *)data;
    // 
    // // Convert to current format:
    // sprout_package_t new_pkg = {0};
...
bool sprout_legacy_handle_protocol_version(code_version_t version, const esp_now_recv_info_t *esp_now_info, const uint8_t *data, int len) 
{
