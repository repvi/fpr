...
/** Current Sprout protocol version (1.0.0 - First LTS Release) */
#define SPROUT_PROTOCOL_VERSION        CODE_VERSION(1, 0, 0)

/** Alias for network version (used in packet headers) */
#define SPROUT_NETWORK_VERSION         SPROUT_PROTOCOL_VERSION

/** Minimum version we support receiving from */
#define SPROUT_MIN_SUPPORTED_VERSION   CODE_VERSION(1, 0, 0)

// ========== VERSION HISTORY ==========

/** Version 0 - Pre-versioning era (legacy devices) */
#define SPROUT_VERSION_LEGACY          CODE_VERSION(0, 0, 0)

/** Version 1.0.0 - First LTS release with host/client modes, security handshake, fragmentation */
#define SPROUT_VERSION_1_0_0           CODE_VERSION(1, 0, 0)

// Future versions:
// #define SPROUT_VERSION_1_1_0        CODE_VERSION(1, 1, 0)  // Extender mode completion
// #define SPROUT_VERSION_1_5_0        CODE_VERSION(1, 5, 0)  // Enterprise security
// #define SPROUT_VERSION_2_0_0        CODE_VERSION(2, 0, 0)  // Breaking change

// ========== FEATURE FLAGS BY VERSION ==========

/** Check if version supports fragmented packets */
#define SPROUT_SUPPORTS_FRAGMENTATION(v)  CODE_VERSION_AT_LEAST(v, SPROUT_VERSION_1_0_0)

/** Check if version supports mesh routing */
#define SPROUT_SUPPORTS_MESH_ROUTING(v)   CODE_VERSION_AT_LEAST(v, SPROUT_VERSION_1_0_0)

/** Check if version has protocol versioning */
#define SPROUT_HAS_VERSIONING(v)          ((v) != SPROUT_VERSION_LEGACY)
 ...
 */
static inline bool sprout_version_is_compatible(code_version_t remote_version)
{
    return CODE_VERSION_AT_LEAST(remote_version, SPROUT_MIN_SUPPORTED_VERSION);
}
 ...
 */
static inline bool sprout_version_is_current(code_version_t remote_version)
{
    // Same major version = compatible, no special handling needed
    return CODE_VERSION_SAME_MAJOR(remote_version, SPROUT_PROTOCOL_VERSION);
}
 ...
 */
static inline bool sprout_version_needs_legacy_handler(code_version_t remote_version)
{
    // Version 0 or older major version needs legacy handling
    return (remote_version == 0) || 
           CODE_VERSION_LESS_THAN(remote_version, CODE_VERSION(CODE_VERSION_MAJOR(SPROUT_PROTOCOL_VERSION), 0, 0));
}
 ...
 */
static inline bool sprout_version_needs_newer_handler(code_version_t remote_version)
{
    return CODE_VERSION_MAJOR(remote_version) > CODE_VERSION_MAJOR(SPROUT_PROTOCOL_VERSION);
}

/**
 * @brief Get the current protocol version
 * @return Current Sprout protocol version
 */
static inline code_version_t sprout_get_current_version(void)
{
    return SPROUT_PROTOCOL_VERSION;
}
