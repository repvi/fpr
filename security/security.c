...
static const char *TAG = "sprout_security";
...
esp_err_t sprout_security_generate_pwk(uint8_t *pwk_out)
{
    ...
    esp_fill_random(pwk_out, SPROUT_KEY_SIZE);
    
    ESP_LOGI(TAG, "Generated new PWK");
    ESP_LOG_BUFFER_HEX_LEVEL(TAG, pwk_out, SPROUT_KEY_SIZE, ESP_LOG_DEBUG);
...
esp_err_t sprout_security_generate_lwk(uint8_t *lwk_out)
{
    ...
    // Generate 16 bytes of cryptographically secure random data
    esp_fill_random(lwk_out, SPROUT_KEY_SIZE);
    
    ESP_LOGI(TAG, "Generated new LWK");
    ESP_LOG_BUFFER_HEX_LEVEL(TAG, lwk_out, SPROUT_KEY_SIZE, ESP_LOG_DEBUG);
...
bool sprout_security_verify_pwk(const uint8_t *pwk_received, const uint8_t *pwk_expected)
{
    ...
    bool match = (memcmp(pwk_received, pwk_expected, SPROUT_KEY_SIZE) == 0);
    
    if (!match) {
        ESP_LOGW(TAG, "PWK verification failed");
        ESP_LOG_BUFFER_HEX_LEVEL(TAG, pwk_received, SPROUT_KEY_SIZE, ESP_LOG_DEBUG);
        ESP_LOG_BUFFER_HEX_LEVEL(TAG, pwk_expected, SPROUT_KEY_SIZE, ESP_LOG_DEBUG);
    } else {
...
bool sprout_security_verify_lwk(const uint8_t *lwk_received, const uint8_t *lwk_expected)
{
    ...
    bool match = (memcmp(lwk_received, lwk_expected, SPROUT_KEY_SIZE) == 0);
    
    if (!match) {
        ESP_LOGW(TAG, "LWK verification failed");
        ESP_LOG_BUFFER_HEX_LEVEL(TAG, lwk_received, SPROUT_KEY_SIZE, ESP_LOG_DEBUG);
        ESP_LOG_BUFFER_HEX_LEVEL(TAG, lwk_expected, SPROUT_KEY_SIZE, ESP_LOG_DEBUG);
    } else {
...
{
    if (keys) {
        memset(keys, 0, sizeof(sprout_security_keys_t));
...
void sprout_security_clear_keys(sprout_security_keys_t *keys)
{
    if (keys) {
        ...
        esp_fill_random(keys->pwk, SPROUT_KEY_SIZE);
        esp_fill_random(keys->lwk, SPROUT_KEY_SIZE);
        memset(keys, 0, sizeof(sprout_security_keys_t));
        keys->pwk_valid = false;
        keys->lwk_valid = false;
