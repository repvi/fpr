/**
 * @file sprout_client.c
 * @brief FPR Client Mode Implementation
 * 
 * Implements client mode functionality including host discovery,
 * connection management, and the client side of the security handshake.
 * 
...
typedef struct {
    ...
    char name[SPROUT_CONNECT_NAME_SIZE];
...
static const char *TAG = "sprout_client";
...
bool sprout_client_is_connected()
{
    bool connected = false;
    size_t count = hashmap_size(sprout_net.peers_map);
    if (count > 0) {
        hashmap_foreach(sprout_net.peers_map, _check_connected_callback, &connected);
    }
    return connected;
}

static void _add_and_ping_host_from_client(const esp_now_recv_info_t *esp_now_info, const sprout_connect_t *info) 
{    
    // Invoke discovery callback if registered
    if (sprout_net.client_config.discovery_cb) {
        sprout_net.client_config.discovery_cb(esp_now_info->src_addr, info->name, esp_now_info->rx_ctrl->rssi);
    }
    ...
    if (!existing && sprout_client_is_connected()) {
        #if (SPROUT_DEBUG == 1)
    ...
    if (sprout_net.client_config.connection_mode == SPROUT_CONNECTION_MANUAL) {
        if (sprout_net.client_config.selection_cb != NULL) {
            bool should_connect = sprout_net.client_config.selection_cb(esp_now_info->src_addr, info->name, esp_now_info->rx_ctrl->rssi);
            if (!should_connect) {
                #if (SPROUT_DEBUG == 1)
    ...
    if (err == ESP_OK) {
        ...
        err = sprout_send_device_info(esp_now_info->src_addr);
        if (err == ESP_OK) {
        ...
        if (sprout_net.client_config.connection_mode == SPROUT_CONNECTION_AUTO) {
...
{
    #if (SPROUT_DEBUG_LOG_CLIENT_DATA_RECEIVE == 1)
    ...
    if (is_broadcast && is_control_packet) {
        ...
        SPROUT_STORE_HASH_TYPE *known_host = _get_peer_from_map(esp_now_info->src_addr);
        if (known_host == NULL) {
            #if (SPROUT_DEBUG == 1)
            ...
            if (!known_host->is_connected || known_host->sec_state != SPROUT_SEC_STATE_ESTABLISHED) {
                // Only reset if we're not already in the middle of a handshake
                if (known_host->sec_state == SPROUT_SEC_STATE_NONE) {
                    ...
                    known_host->sec_state = SPROUT_SEC_STATE_NONE;
                    known_host->is_connected = false;
                    known_host->state = SPROUT_PEER_STATE_DISCOVERED;
                    ...
                    if (sprout_net.client_config.connection_mode == SPROUT_CONNECTION_AUTO) {
                        esp_err_t err = sprout_send_device_info(esp_now_info->src_addr);
                        if (err == ESP_OK) {
                        ...
                        if (sprout_net.client_config.selection_cb) {
                            bool should_connect = sprout_net.client_config.selection_cb(esp_now_info->src_addr, info->name, esp_now_info->rx_ctrl->rssi);
                            if (should_connect) {
                                esp_err_t err = sprout_send_device_info(esp_now_info->src_addr);
                                if (err == ESP_OK) {
        ...
        if (existing) {
            ...
            if (is_control_packet) {
                if (info->has_pwk && !info->has_lwk) {
                    ...
                    if (existing->sec_state >= SPROUT_SEC_STATE_LWK_SENT) {
                    ...
                    if (existing->sec_state < SPROUT_SEC_STATE_PWK_RECEIVED) {
                        // Generate own LWK and send PWK+LWK back
                        sprout_sec_client_handle_pwk(esp_now_info->src_addr, existing, info);
                    }
                    #if (SPROUT_DEBUG == 1)
                    else {
                        ESP_LOGW(TAG, "Ignoring duplicate PWK - already in handshake (current_state=%d, expected<%d)", 
                                 existing->sec_state, SPROUT_SEC_STATE_PWK_RECEIVED);
                    }
                    #endif
                    ...
                    if (existing->sec_state == SPROUT_SEC_STATE_LWK_SENT) {
                        ...
                        sprout_sec_client_verify_ack(esp_now_info->src_addr, existing, info);
                    }
                    #if (SPROUT_DEBUG == 1)
                    else {
                        ESP_LOGW(TAG, "Ignoring ACK - wrong state (current=%d, expected=%d, has_pwk=%d, has_lwk=%d)", 
                                 existing->sec_state, SPROUT_SEC_STATE_LWK_SENT, info->has_pwk, info->has_lwk);
                    }
                    #endif
                }
            }
...
void _find_host_callback(void *key, void *value, void *user_data)
{
    ...
    if (peer && peer->state >= SPROUT_PEER_STATE_DISCOVERED && !ctx->found) {
...
esp_err_t sprout_client_get_host_info(uint8_t *mac_out, char *name_out, size_t name_size)
{
    ESP_RETURN_ON_FALSE(mac_out != NULL, ESP_ERR_INVALID_ARG, TAG, "MAC output buffer is NULL");

    host_search_ctx_t ctx = {
        .name_size = (name_size > 0 && name_size < SPROUT_CONNECT_NAME_SIZE) ? name_size : SPROUT_CONNECT_NAME_SIZE,
        .found = false
    };

    hashmap_foreach(sprout_net.peers_map, _find_host_callback, &ctx);
...
{
    sprout_net.client_config.connection_mode = mode;
    ESP_LOGI(TAG, "Client connection mode set to %s", mode == SPROUT_CONNECTION_AUTO ? "AUTO" : "MANUAL");
    return ESP_OK;
}

void sprout_client_register_discovery_callback(sprout_peer_discovered_cb_t cb)
{
    sprout_net.client_config.discovery_cb = cb;
    ESP_LOGI(TAG, "Client discovery callback %s", cb ? "registered" : "unregistered");
}

void sprout_client_register_selection_callback(sprout_host_selection_cb_t cb)
{
    sprout_net.client_config.selection_cb = cb;
    ESP_LOGI(TAG, "Client selection callback %s", cb ? "registered" : "unregistered");
}
...
static void _filter_peers_callback(void *key, void *value, void *user_data)
{
    ...
    if (ctx->count < ctx->max_peers && peer) {
        ...
        if (ctx->connected_only && peer->state != SPROUT_PEER_STATE_CONNECTED) {
...
size_t sprout_client_list_discovered_hosts(sprout_peer_info_t *peer_array, size_t max_peers)
{
    ...
    hashmap_foreach(sprout_net.peers_map, _filter_peers_callback, &ctx);
    return ctx.count;
}

esp_err_t sprout_client_connect_to_host(uint8_t *peer_mac, TickType_t timeout)
{
    ...
    const TickType_t retry_interval = pdMS_TO_TICKS(SPROUT_MANUAL_CONNECT_RETRY_INTERVAL_MS);
...
esp_err_t sprout_client_disconnect(void)
{
    // Find and disconnect from connected host
    uint8_t host_mac[6];
    esp_err_t err = sprout_client_get_host_info(host_mac, NULL, 0);
    if (err == ESP_OK) {
        SPROUT_STORE_HASH_TYPE *peer = _get_peer_from_map(host_mac);
        if (peer) {
...
size_t sprout_client_scan_for_hosts(TickType_t duration)
{
    ESP_LOGI(TAG, "Scanning for hosts for %lu ms", (unsigned long)pdTICKS_TO_MS(duration));
    
    size_t initial_count = hashmap_size(sprout_net.peers_map);
    TickType_t start = xTaskGetTickCount();
    const TickType_t broadcast_interval = pdMS_TO_TICKS(SPROUT_HOST_SCAN_BROADCAST_INTERVAL_MS);
    TickType_t last_broadcast = 0;
    
    while (xTaskGetTickCount() - start < duration) {
        TickType_t now = xTaskGetTickCount();
        if (now - last_broadcast >= broadcast_interval) {
            extern esp_err_t sprout_broadcast_device_info();
            sprout_broadcast_device_info();
            last_broadcast = now;
        }
        vTaskDelay(pdMS_TO_TICKS(SPROUT_HOST_SCAN_POLL_INTERVAL_MS));
    }
    
    size_t final_count = hashmap_size(sprout_net.peers_map);
...
void _client_reconnect_task(void *arg)
{
    (void)arg;
    ...
    while (1) {
        ...
        const TickType_t check_interval_ticks = pdMS_TO_TICKS(_sprout_get_power_adjusted_interval(SPROUT_CLIENT_WAIT_CHECK_INTERVAL_MS));
        
        // If connected, send periodic keepalive and check for host timeout
        uint8_t host_mac[MAC_ADDRESS_LENGTH];
        if (sprout_client_get_host_info(host_mac, NULL, 0) == ESP_OK) {
            SPROUT_STORE_HASH_TYPE *host_peer = _get_peer_from_map(host_mac);
            if (host_peer && host_peer->is_connected) {
                // keepalive send periodically
                if ((xTaskGetTickCount() - last_keep) >= keep_interval_ticks) {
                    esp_err_t err = sprout_send_device_info(host_mac);
                    if (err != ESP_OK) {
                ...
                uint32_t timeout_ms = _sprout_get_power_adjusted_interval(SPROUT_RECONNECT_TIMEOUT_MS);
                if ((uint64_t)US_TO_MS(age_us) > timeout_ms) {
                    ...
                    host_peer->state = SPROUT_PEER_STATE_DISCOVERED;
                }
            }
        }
