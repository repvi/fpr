...
static const char *TAG = "sprout_lts";
...
void sprout_lts_log_compatibility(code_version_t remote_version)
{
    ...
    sprout_lts_version_to_string(SPROUT_PROTOCOL_VERSION, local_str, sizeof(local_str));
    sprout_lts_version_to_string(remote_version, remote_str, sizeof(remote_str));
    
    if (sprout_version_is_compatible(remote_version)) {
        if (sprout_version_is_current(remote_version)) {
            ESP_LOGI(TAG, "Remote version %s is compatible (same major version as local %s)", 
                     remote_str, local_str);
        } else if (sprout_version_needs_legacy_handler(remote_version)) {
            ESP_LOGW(TAG, "Remote version %s requires legacy handler (local: %s)", 
                     remote_str, local_str);
        } else if (sprout_version_needs_newer_handler(remote_version)) {
...
{
    return SPROUT_MIN_SUPPORTED_VERSION;
}

/**
 * @brief Check if a feature is supported by a given version
 * @param version Version to check
 * @param feature Feature identifier (use SPROUT_SUPPORTS_* macros for known features)
...
bool sprout_lts_supports_feature(code_version_t version, const char *feature)
{
    ...
    if (strcmp(feature, "fragmentation") == 0) {
        return SPROUT_SUPPORTS_FRAGMENTATION(version);
    }
    if (strcmp(feature, "mesh_routing") == 0) {
        return SPROUT_SUPPORTS_MESH_ROUTING(version);
    }
    if (strcmp(feature, "versioning") == 0) {
        return SPROUT_HAS_VERSIONING(version);
    }
